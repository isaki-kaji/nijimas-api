...
const getPostsByMainCategoryAndSubCategory = `-- name: GetPostsByMainCategoryAndSubCategory :many
SELECT
  p.post_id,
  u.uid,
  u.username,
...
JOIN (
  ...
  GROUP BY ps.post_id
  HAVING 
    MAX(CASE WHEN ps.category_no = '1' THEN s.category_name ELSE NULL END) = $3 OR 
    MAX(CASE WHEN ps.category_no = '2' THEN s.category_name ELSE NULL END) = $3 
) sc ON p.post_id = sc.post_id
LEFT JOIN favorites f
  ON p.post_id = f.post_id AND f.uid = $1
...
type GetPostsByMainCategoryAndSubCategoryParams struct {
	Uid          string `json:"uid"`
	MainCategory string `json:"main_category"`
	SubCategory  string `json:"sub_category"`
}

type GetPostsByMainCategoryAndSubCategoryRow struct {
	PostID          uuid.UUID       `json:"post_id"`
	Uid             string          `json:"uid"`
	Username        string          `json:"username"`
	ProfileImageUrl *string         `json:"profile_image_url"`
...
func (q *Queries) GetPostsByMainCategoryAndSubCategory(ctx context.Context, arg GetPostsByMainCategoryAndSubCategoryParams) ([]GetPostsByMainCategoryAndSubCategoryRow, error) {
	rows, err := q.db.Query(ctx, getPostsByMainCategoryAndSubCategory, arg.Uid, arg.MainCategory, arg.SubCategory)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPostsByMainCategoryAndSubCategoryRow{}
	for rows.Next() {
		var i GetPostsByMainCategoryAndSubCategoryRow
		if err := rows.Scan(
			&i.PostID,
			&i.Uid,
			&i.Username,
